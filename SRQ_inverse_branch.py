...
def inverse_with_phi(x, r, phi, p_bs, p_se, p_ew, p_wt, lamda):
    ...
    len_3_se = np.array([0, p_se, 0])
    len_4_ew = np.array([0, 0, p_ew])
    len_7_wt = np.array([0, 0, p_wt])
    x_0_sw = x - len_0_bs - r @ len_7_wt
    theta_4 = np.arccos((np.linalg.norm(x_0_sw)**2 - p_se**2 - p_ew**2) / (2 * p_se * p_ew))
...
def inverse_paramtric_matrix(x, r, p_bs, p_se, p_ew, p_wt):
    ...
    len_7_wt = np.array([0, 0, p_wt])
    x_0_sw = x - len_0_bs - r @ len_7_wt
    cos_4 = (np.linalg.norm(x_0_sw)**2 - p_se**2 - p_ew**2) / (2 * p_se * p_ew)
    if cos_4 > 1 or cos_4 < -1:
        return None
    else:
        theta_4 = np.arccos(cos_4)
        u_0_sw = x_0_sw / np.linalg.norm(x_0_sw)
...
def phi_calculation(lamda, M_w, choice):
    #/ 两种算法都可以,前一种容易避开奇异点，亲测后决定取sqrt法
    upper = (M_w[0][2, 1], M_w[1][2, 1], M_w[2][2, 1])
    lower = (M_w[0][2, 0], M_w[1][2, 0], M_w[2][2, 0])
    eta = np.radians(lamda)
    coefficient = np.array(upper) + np.tan(eta) * np.array(lower)
...
def inverse_with_lamda(x, r, lamda, choice):
    ...
    if M_w is not None:
        ...
        if phi is not None:
            ...
            result[5] -= beta
            # result[6] = np.radians(lamda)
            # print('Solvable:{0}'.format(lamda))
            return result
        else:
...
def inverse_kinematics(x, r, lamda, branch):
    ...
    # if branch == 1:
    #     if lamda < 10:
    #         return inverse_with_lamda(x, r, lamda, 1)
    #     else:
    #         result1 = inverse_with_lamda(x, r, lamda, 1)
    #         result2 = inverse_with_lamda(x, r, lamda, -1)
    #         miu = lamda - 10
    #         solvable = False
    #         near = []
    #         while miu > 0:
    #             near = inverse_kinematics(x, r, miu, 1)
    #             if near is not None:
    #                 solvable = True
    #                 break
    #             miu -= 10
    #         if solvable:
    #             dis1 = np.linalg.norm(near - result1)
    #             dis2 = np.linalg.norm(near - result2)
    #             return result1 if dis1 < dis2 else result2
    #         else:
    #             return None
    ...
    if result_neg[6] == np.radians(lamda):
        return result_neg
    elif result_pos[6] == np.radians(lamda):
        return result_pos
    else:
        return None
...
if __name__ == '__main__':
    ...
    # x = x_07_d
    # r = r_07_d
    reference = fk_kuka([-90, 9, -4, -90, 2, 3, 30])
    x = reference[:3, 3]
    r = reference[:3, :3]
    ...
    # real_phi = []
    # real_phi2 = []
    # angle = inverse_with_lamda(x, r, 190, -1)  #TODO 查看公式，双输入反正切其实依赖角6，角2的sin
    # ic(np.degrees(angle))
    # ic(fk_kuka(angle))
    # ic(x)
    ...
    for phi in range(0, 360, 2):
        if phi == 90 or phi == 180 or phi == 270:
            #- 奇异点需要单独解法
            continue
        ...
        if angle is not None:
            phi_set.append(phi)
            if phi <= 270 and phi >= 90:
                joints.append(angle)
                joints2.append(angle2)
            else:
                joints.append(angle2)
                joints2.append(angle)             
