...
def inverse_with_phi(x, r, phi, p_bs, p_se, p_ew, p_wt, lamda, choice, positive):
    ...
    if theta_4 is None:
        ...
        #! 计算theta1,2,3, 多值
        #TODO 此处有分支,取决于theta2的正负
        #TODO theta2 = 0, 180时讨论，由于franka角度限制，只讨论0即可
...
def inverse_with_lamda(x, r, lamda, choice, positive=True):
    ...
    if M_w is not None:
        ...
        if phi is not None:
            result = inverse_with_phi(x, r_srs, phi, d_bs, d_se, d_ew, sevenlink, lamda, choice, positive)
            result[5] -= beta
            return normalize_theta_output(result)
        else:
...
if __name__ == '__main__':
    ...
    # angle = inverse_with_lamda(x, r, -65, 1)  #TODO 查看公式，双输入反正切其实依赖角6，角2的sin
    # ic(np.degrees(angle))
    # ic(fk_kuka(angle))
    # ic(x)
